...
int main( void ) {
	...
	while ( capture.read( frame ) ) {
		...
		putText(frame, "Detected : "+ std::to_string(v), cv::Point(20, 50), cv::FONT_HERSHEY_DUPLEX, 1.0, {0, 255, 0, 255});


		putText(frame, "Detection Counter : "+ std::to_string(z), cv::Point(20, 150), cv::FONT_HERSHEY_DUPLEX, 1.0, {0, 255, 0, 255});

			 if (detected_eyes.size() > 1) {
            Rect table[2] = {Rect(), Rect()};

            for(int i =0; i<detected_eyes.size();i++){
                Rect r = boundingRect(detected_eyes);
                int area = r.area();
